...
const TrainingProgram = () => {
  ...
  const { subjects } = useAppSelector((state) => state.training);
      ...
      {Object.entries(groupedByNote).map(([note, list]) => {
        ...
        return (
          <div key={note} className="mb-10">
            ...
            {Object.entries(groupedBySemester).map(([semester, subjectsInSemester]) => (
              <div key={semester} className="mb-6">
                <h3 className="text-md font-semibold text-gray-700 mb-2">Học kỳ {semester}</h3>
                <table className="w-full text-sm text-left border rounded overflow-hidden bg-white dark:bg-gray-800 dark:text-white">
                  ...
                  <tbody>
                    {subjectsInSemester.map((subject) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">{subject.theory_hours}</td>
                        <td className="px-3 py-2">{subject.practice_hours}</td>
                        <td className="px-3 py-2">
                          {subject.is_optional ? "Tự chọn" : "Bắt buộc"}
                        </td>
